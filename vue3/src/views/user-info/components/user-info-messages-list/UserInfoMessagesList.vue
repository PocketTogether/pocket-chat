...
} = props.userInfoQueryDesuwa

// 分页控制
const canGoPreviousPage = computed(() => {
  if (
    userInfoMessageListQuery.data.value?.totalPages == null ||
    userInfoMessageListQuery.data.value?.totalPages <= 0
  ) {
    return false
  }
  if (userInfoMessageListPageNum.value - 1 <= 0) {
    return false
  }
  return true
})
const goPreviousPage = () => {
  ...
  userInfoMessageListPageSet(userInfoMessageListPageNum.value - 1)
}
const canGoNextPage = computed(() => {
  ...
  if (
    ...
    userInfoMessageListQuery.data.value?.totalPages
  ) {
    return false
  }
  return true
...
<template>
  <div v-if="userInfoMessageListQuery.data.value != null">
    <div class="overflow-hidden rounded-[24px] bg-color-background-soft">
      ...
      <div class="mx-[8px] my-[8px]">
        <div class="flex items-center justify-between">
          ...
          <div class="ml-[12px] flex-1 truncate">
            ...
            >
              {{ i18nStore.t('userInfoPageMessageListTitle')() }}
            </div>
          </div>
          <!-- 右 个数 -->
          <div>
            <div class="flex items-center">
              <div class="select-none text-[14px] font-bold text-color-text">
                {{ userInfoMessageListQuery.data.value.totalItems }}
              </div>
              <div class="ml-[0px]">
                <div class="flow-root">
      ...
      <div class="relative">
        <!-- 消息项 -->
        <div
          v-for="item in userInfoMessageListQuery.data.value.items"
          :key="item.id"
        >
          <!-- 分割线 -->
        ...
        <div
          v-if="
            userInfoMessageListQuery.data.value.totalPages > 1 &&
            userInfoMessageListQuery.data.value.items.length < 2
          "
        >
          <!-- 分割线 -->
          <div class="border-t-[3px] border-color-background"></div>
        ...
        <!-- <Transition name="fade500ms" mode="out-in">
          <div
            v-if="
              userInfoMessageListQuery.isFetching.value === true &&
              // 有足够空间时才显示
              userInfoMessageListQuery.data.value.totalItems >= 2
            "
      ...
      <!-- 分页栏在需要时才显示 -->
      <template v-if="userInfoMessageListQuery.data.value.totalPages > 1">
        <!-- 分割线 -->
        <div class="border-t-[3px] border-color-background"></div>
        <!-- 分页栏 -->
        <div>
              ...
              <div class="page-button-scrollbar flow-root h-full">
                ...
                >
                  <div class="flex h-full w-fit items-stretch">
                    <template
                      v-for="item in userInfoMessageListQuery.data.value
                        .totalPages"
                      :key="item"
                    >
                      <div
                        ...
                        :class="{
                          'bg-el-primary-light-6':
                            userInfoMessageListPageNum === item,
                        }"
                        @click="userInfoMessageListPageSet(item)"
                      >
                        <div
