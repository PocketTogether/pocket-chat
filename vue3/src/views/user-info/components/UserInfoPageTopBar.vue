...
const { userQueryRefresh, isUserQueryRefreshRunning } =
  props.userInfoQueryDesuwa

// 让加载动画至少显示1秒（转一圈），且转的圈数为整数
const { sourceHaveHold: isUserQueryRefreshRunningForAni } =
  useWatchSourceToHoldTimeAndStep({
    source: computed(() => isUserQueryRefreshRunning.value),
    holdMs: 1000,
    stepMs: 1000,
  })
    ...
    <Transition name="fade-down-up">
      ...
      >
        ...
        <ChatTopBarMoreMenuItem
          :isRunning="isUserQueryRefreshRunningForAni"
          :isRunnable="true"
          @click="userQueryRefresh"
        >
          <template #icon>
            <RiRestartLine
              size="18px"
              :class="{
                'loading-spinner-1s': isUserQueryRefreshRunningForAni,
              }"
            ></RiRestartLine>
          </template>
          <template #text>
            {{ i18nStore.t('userInfoPageUserQueryRefreshText')() }}
          </template>
        </ChatTopBarMoreMenuItem>
        <!-- 收起 -->
        <div
