...
const { sourceHaveHold: isSearchMessageQueryRefreshRunningForAni } =
  useWatchSourceToHoldTimeAndStep({
    source: computed(() => isSearchMessageQueryRefreshRunning.value),
    holdMs: 1000,
    stepMs: 1000,
  })
  ...
  <div class="relative">
    ...
    <Transition name="fade-down-up">
      ...
      >
        ...
        <!-- 菜单项 刷新 -->
        <ChatTopBarMoreMenuItem
          :isRunning="isSearchMessageQueryRefreshRunningForAni"
          :isRunnable="true"
          @click="searchmessageQueryRefresh()"
        >
          <template #icon>
            <RiRestartLine
              size="18px"
              :class="{
                'loading-spinner-1s': isSearchMessageQueryRefreshRunningForAni,
              }"
            ></RiRestartLine>
          </template>
          <template #text>
            {{ i18nStore.t('searchPageSearchQueryRefreshText')() }}
          </template>
        </ChatTopBarMoreMenuItem>
        <!-- 收起 -->
        <div
