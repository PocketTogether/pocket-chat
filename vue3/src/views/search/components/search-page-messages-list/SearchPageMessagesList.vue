...
const isFetching = computed(() => searchmessagePageListQuery.isFetching.value)
...
const canGoPreviousPage = computed(() => {
  ...
  // }
  if (
    searchmessagePageListQuery.data.value?.totalPages == null ||
    searchmessagePageListQuery.data.value?.totalPages <= 0
  ) {
    return false
  }
  if (searchmessageQueryPage.value - 1 <= 0) {
    return false
  }
  return true
})
const goPreviousPage = () => {
  ...
  searchmessageQueryPageSet(searchmessageQueryPage.value - 1)
}
const canGoNextPage = computed(() => {
  ...
  if (
    ...
    searchmessagePageListQuery.data.value?.totalPages
  ) {
    return false
  }
  return true
...
// const canGoNumPage = computed(() => {
//   if (isFetching.value === true) {
//     return false
//   }
//   return true
// })
// const goNumPage = (pageNum: number) => {
//   if (canGoNumPage.value === false) {
//     return
//   }
//   searchmessageQueryPageSet(pageNum)
// }
...
<template>
  <div v-if="searchmessagePageListQuery.data.value != null">
    <div class="overflow-hidden rounded-[24px] bg-color-background-soft">
      ...
      <div class="mx-[8px] my-[8px]">
        <div class="flex items-center justify-between">
          ...
          <div class="ml-[12px] flex-1 truncate">
            ...
            >
              {{ i18nStore.t('searchPageMessageListTitle')() }}
            </div>
          </div>
          <!-- 右 个数 -->
          <div>
            <div class="flex items-center">
              <div class="select-none text-[14px] font-bold text-color-text">
                {{ searchmessagePageListQuery.data.value.totalItems }}
              </div>
              <div class="ml-[0px]">
                <div class="flow-root">
                  <Transition mode="out-in" name="fade">
                    <div
                      v-if="isFetchingForAni"
                      class="m-[4px] text-color-text"
                    >
                      <RiLoader3Line
                    ...
                    <div v-else class="m-[4px] text-color-text">
      ...
      <div class="relative">
        <!-- 消息项 -->
        <div
          v-for="item in searchmessagePageListQuery.data.value.items"
          :key="item.id"
        >
          <!-- 分割线 -->
        ...
        <div
          v-if="
            searchmessagePageListQuery.data.value.totalPages > 1 &&
            searchmessagePageListQuery.data.value.items.length < 2
          "
        >
          <!-- 分割线 -->
          <div class="border-t-[3px] border-color-background"></div>
        ...
        <!-- <Transition name="fade500ms" mode="out-in">
          <div
            v-if="
              searchmessagePageListQuery.isFetching.value === true &&
              // 有足够空间时才显示
              searchmessagePageListQuery.data.value.totalItems >= 2
            "
      ...
      <!-- 分页栏在需要时才显示 -->
      <template v-if="searchmessagePageListQuery.data.value.totalPages > 1">
        <!-- 分割线 -->
        <div class="border-t-[3px] border-color-background"></div>
        <!-- 分页栏 -->
        <div>
              ...
              <div class="page-button-scrollbar flow-root h-full">
                ...
                >
                  <div class="flex h-full w-fit items-stretch">
                    <template
                      v-for="item in searchmessagePageListQuery.data.value
                        .totalPages"
                      :key="item"
                    >
                      <div
                        ...
                        :class="{
                          'bg-el-primary-light-6':
                            searchmessageQueryPage === item,
                        }"
                        @click="searchmessageQueryPageSet(item)"
                      >
                        <div
