...
默认为0
为 0 时系统将根据 config 集合中的 user-max-upload-file-size-default 配置决定所能上传文件的最大大小
注意，maxUploadFileSize 或 user-max-upload-file-size-default 都仅仅是给前端提供数据，是由前端来进行大小限制的，因为pocketbase暂不支持在api规则中限制文件大小
