...
```
有新消息时网站图标与标题改变
收到消息、发送休息、修改消息 都增加了音效提示
发送消息快捷键从 shift+enter 改为 alt+enter
实现桌面消息通知
实现pb实时订阅断线重连时消息补偿
```
...
- project-tools-node/
  ...
  - jsconfig.json
- pocketbase-release-file/
  - pocketbase_0.33.0_windows_amd64.zip
  - pocketbase_0.33.0_linux_amd64.zip
  - pocketbase_0.33.0_linux_arm64.zip
  - ……
  - pocketbase_0.33.0_windows_amd64/
  - pocketbase_0.33.0_linux_amd64/
  - pocketbase_0.33.0_linux_arm64/
...
project-tools-node/ 中会写一些js脚本 用于项目 打包 之类的操作
pocketbase-release-file/ 为存放不同平台的pocketbase可执行文件，以供以后使用

project-tools-node 中的脚本，需按 jsconfig.json 中的规则：
{
  "compilerOptions": {
    "checkJs": true,
    "strict": true,
...
project-config.js 其中为项目配置信息，将被其他脚本导入使用
```js
// === 可配置变量 ===
/** 
 * 项目名
...
project-pocketbase-download.js 为此次任务中需要编写的脚本
```
目的是下载各个 pocketbase 发布的 .zip 文件到 pocketbase-release-file 并解压
下载前需清空 pocketbase-release-file/
注意在文件操作时，不要因为类似 创建或复制或下载，文件或文件夹时 a/b/c/ 时中间目录不存在的原因而导致错误。
避免将代码复杂化。
使用 extract-zip 来解压
使用 fetch 来下载，多文件下载时要异步
使用 fs 来进行同步的文件操作
```
...
```
TODO
.gitattributes # Windows 批处理脚本当作二进制文件，以避免进行换行符转换
README.md 完善，补上英文文档的图片遗漏

pocketbase\start_docker.sh
project-tools-node 脚本更新
Dockerfile 更新
